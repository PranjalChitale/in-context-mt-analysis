...
n_shots=(1 4 8)
instruction_types=("generic" "random" "standard" "contrastive" "no_instruct")


# --------------------------------------------------------------------------
#                       Instruct-variant in-context eval
# --------------------------------------------------------------------------
...
for n_shot in "${n_shots[@]}"; do
    ...
    for instruction_type in "${instruction_types[@]}"; do
        ...
        echo "Running ${instruction_type} variant in-context eval for ${src_lang}-${tgt_lang} for ${n_shot} shot"
